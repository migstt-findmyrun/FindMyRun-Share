...
export default async function handler(req, res) {
    ...
    try {
        console.log('Fetching club id:', id, '| key present:', !!anonKey);

        const response = await fetch(
            `${SUPABASE_URL}/rest/v1/clubs?id=eq.${encodeURIComponent(id)}&select=*&limit=1`,
            {
                headers: {
                    'apikey': anonKey,
        ...
        const clubs = JSON.parse(rawText);
        const club = clubs?.[0];
        ...
        res.status(200).send(clubPage(club));
    } catch (err) {
        res.setHeader('Content-Type', 'text/html; charset=utf-8');
...
function clubPage(club) {
    const name = escHtml(club.name || 'Running Club');
    const location = [club.city, club.state, club.country].filter(Boolean).map(escHtml).join(', ');
    const description = club.description ? escHtml(club.description) : null;
    const memberCount = club.member_count ? club.member_count.toLocaleString() : null;
    const stravaURL = club.strava_id
        ? `https://www.strava.com/clubs/${club.strava_id}`
        : club.slug
        ? `https://www.strava.com/clubs/${club.slug}`
        : null;
...
<head>
  ...
  <title>${name} — FindMyRun</title>
  <meta property="og:title" content="${name}">
  <meta property="og:description" content="${location ? location + ' · ' : ''}${memberCount ? memberCount + ' members' : 'Running club'}">
...
<body>
  ...
  <div class="card">
    <div class="club-label">Running Club</div>
    ...
    ${memberCount ? `<div class="detail-row">
      <span class="detail-icon">👥</span>
      <span class="detail-text">${memberCount} members</span>
    </div>` : ''}
    ${stravaURL ? `<div class="detail-row">
      <span class="detail-icon">🏃</span>
      <span class="detail-text"><a href="${stravaURL}" style="color:#fc4c02;text-decoration:none;">View on Strava</a></span>
    </div>` : ''}
...
<head>
  ...
  <title>Club Not Found — FindMyRun</title>
  <style>
...
<body>
  ...
  <p>This club may have been removed.</p>
